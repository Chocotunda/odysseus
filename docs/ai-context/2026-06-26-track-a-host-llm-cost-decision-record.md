...
- **TWO ACCESS LANES for the SAME model (decided 2026-06-27):**
  - **TEST lane = OpenRouter free** (`deepseek/deepseek-v4-flash:free`, base `https://openrouter.ai/api/v1`). ~~**$0**, 1M ctx.~~ **⚠️ RETIRED 2026-06-28 — DO NOT USE.** The `:free` slug now 404s (`"This model is unavailable for free… use deepseek/deepseek-v4-flash"`); a fallback free model (Mistral) 404'd the same way, i.e. free access appears revoked for the account. **The free test-lane is gone** — the e2e workflow (`wf_c40218f1-28f`) confirmed it on the first live call. The Odysseus-side wiring was otherwise verified sound (resolve→decrypt→build_headers→stream_llm→SSE parse), the spend-cap/cost plumbing passed offline, and the native-tool-call→ToolBlock parse passed offline; only a successful **live** generation is unproven. **→ Phase-1 live testing must use a PAID lane** (DeepSeek-direct preferred, or OpenRouter-paid `deepseek/deepseek-v4-flash`) **or a local Ollama tool-capable model**. (Originally: 50 req/day → 1,000/day after a $10 top-up; free routing auto-failovers across providers so cache continuity breaks — moot now.)
  - **PROD lane = paid metered** for the always-on VPS brain + Hermes. Free can't sustain production: one `agent_loop` task fires up to **50 req** (`MAX_AGENT_ROUNDS=50`), so 50/day ≈ one task, 1,000/day ≈ ~20 tasks; a 24/7 Hermes loop blows through it. **RECOMMENDED: OpenRouter-paid `deepseek/deepseek-v4-flash` (researched 2026-06-28).** Corrects two earlier assumptions: (a) **OpenRouter does NOT break cache continuity** — it uses **sticky routing** (after a cached request it pins the same provider to keep the cache warm, auto, *unless* you manually set `provider.order`); DeepSeek caching is automatic, no `cache_control`. (b) **OpenRouter's ToS permits autonomous/agent use** (bars only reselling + website scraping; must still honor DeepSeek's model terms) — so it's fine for Hermes. Plus: **no inference markup** (provider price passthrough; only a **5.5% fee on credit top-ups**), DeepSeek V4-Flash listed *cheaper* on miss (**$0.09/$0.18** vs DeepSeek-direct **$0.14/$0.28**), one account/key for ~400 models (swap-friendly), failover resilience, and **already wired + key already added**. **The ONE thing to MEASURE in the soak:** OpenRouter's *cache-read depth* for DeepSeek — DeepSeek-direct's native hit is **$0.0028/1M (~98% off)**; OpenRouter advertises a blended "60–80% cheaper", and the deep $0.0028 read only applies if routed to DeepSeek's own provider endpoint. If the measured effective cost on the heavy Hermes path is materially worse than the modeled ~$0.0028 cached, **fall back to DeepSeek-direct** (`api.deepseek.com`) for that path only. Don't open a second account preemptively.
...
- **✅ LIVE-VERIFIED 2026-06-28 (OpenRouter-paid `deepseek/deepseek-v4-flash`, endpoint `0e53e031`).** Real calls through `stream_llm`: (1) chat completion returned clean text + usage (served `deepseek-v4-flash-20260423`); (2) **tool round-trip** — the model emitted a native `tool_call` (`get_current_time({"timezone":"UTC"})`) whose shape parses through `agent_loop.function_call_to_tool_block` for registered tools (confirmed with `read_file`; unknown tools correctly rejected). Metered accounting confirmed on real usage (`compute_cost` priced 2 calls at **$0.0000974** total; `is_local_endpoint`→metered). **Phase-1 integration = DONE.** Open follow-up = **measure cache-read depth** over a week (these fresh calls surfaced no cache-token fields — expected on a cold call); enable `ODYSSEUS_DAILY_LLM_BUDGET_USD` for the soak.
