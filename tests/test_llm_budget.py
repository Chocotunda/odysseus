...
def test_daily_rollover_resets_spend(monkeypatch):
    """Spend on day A must not appear when day_key rolls over to day B."""
    _reset(monkeypatch, 100.0)

    day_a = "2099-01-01"
    day_b = "2099-01-02"

    # Clean up both days from any prior run
    db = SessionLocal()
    db.query(DailyLLMSpend).filter(DailyLLMSpend.day.in_([day_a, day_b])).delete(synchronize_session=False)
    db.commit()
    db.close()

    try:
        # Record spend on day A
        monkeypatch.setattr(budget, "day_key", lambda: day_a)
        budget.add_spend(5.0)
        assert round(budget.today_spend(), 2) == 5.0

        # Roll over to day B — today_spend() must be 0
        monkeypatch.setattr(budget, "day_key", lambda: day_b)
        assert budget.today_spend() == 0.0
    finally:
        # Clean up both rows regardless of test outcome
        db = SessionLocal()
        db.query(DailyLLMSpend).filter(DailyLLMSpend.day.in_([day_a, day_b])).delete(synchronize_session=False)
        db.commit()
        db.close()
